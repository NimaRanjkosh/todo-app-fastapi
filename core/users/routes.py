...
async def user_login(
    ...
    fetched_user = (
        db.query(UserModel).where(UserModel.username == user_info.username).first()
    )
    if not fetched_user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"username: '{user_info.username}' doesn't exists. Register first.",
        )
...
async def user_register(
    ...
    if query:
        raise HTTPException(
            ...
            detail=f"username: {user_info.username} already exists. Try Login instead of register",
        )
...
async def user_register(
    ...
    if not query:
        raise HTTPException(
            ...
            detail=f"username: {user_id} not exists.",
        )
